...
def sync_to_cloud():
    ...
    for row in rows:
        ...
        print(f"📤 Published transaction {record.get('transaction_id', '(no id)')}")
...
def on_message(client, userdata, msg):
    ...
    try:
        image_path = msg.payload.decode().strip()
        print(f"📥 Received image request for: {image_path}")

        if not os.path.exists(image_path):
            error_msg = json.dumps({"error": "File not found", "path": image_path})
            client.publish(RESPONSE_TOPIC, error_msg)
        ...
        print(f"📤 Sent image data to topic '{RESPONSE_TOPIC}'")

    except Exception as e:
        error_payload = json.dumps({"error": str(e), "path": image_path})
