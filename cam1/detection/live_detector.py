...
MODEL_PATH  = str(BASE / get("CAM1_MODEL",    "cam1/models/jl_yolox_cam1.pth"))
EXP_FILE    = str(BASE / get("CAM1_EXP_FILE", "cam1/YOLOX/exps/default/yolox_s.py"))
NUM_CLASS   = getint("CAM1_NUM_CLASSES",  7)
...
INFERENCE_MODE = get("CAM1_INFERENCE_MODE",  "grayscale")
...
CLASS_NAMES = _load_class_names("CAM1_CLASS_NAMES")

from core.config import getmap
BOX_CLASSES  = getmap("CAM1_BOX_CLASSES")
BALE_CLASSES = getmap("CAM1_BALE_CLASSES")
CLASS_WEIGHTS = {**{k: 1 for k in BOX_CLASSES}, **{k: 1 for k in BALE_CLASSES}}
...
class LiveDetector:
                ...
                if tid in prev_x:
                    ...
                    if crossed and tid not in counted_ids:
                        ...
                        if best_cls in CLASS_WEIGHTS:
                            counted_ids.add(tid)
                            weight = (BOX_CLASSES.get(best_cls)
                                      or BALE_CLASSES.get(best_cls, 1))
                            counter = ("box" if best_cls in BOX_CLASSES
                                       else "bale")
    ...
    def _update_db(self):
        ...
        # Also publish offset counts for all active virtual sessions
        try:
            from cam1.api.virtual_session import publish_virtual_counts
            publish_virtual_counts(self.counts)
        except Exception:
            pass
