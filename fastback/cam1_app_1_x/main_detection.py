...
from config_loader import (
    ...
    BALE_CLASSES,
    RL_SAVE_DIR,           # ← ADD
    RL_CONF_THRESHOLD,     # ← ADD
    RL_LINE_PROXIMITY_PX,  # ← ADD
)
...
# -------------------------------------------------
CLASS_NAMES = {
    0: "box-Sedr",   # spurious — ignored by CLASS_WEIGHTS gate
    1: "bale",
    2: "box",
    3: "fbale",
    4: "sbale",
...
async def detect_objects(video_source: str, session_id: str):
    ...
    try:
        ...
        logger.info(f"DIAG: VideoCapture opened. Backend={cv2.CAP_FFMPEG} BufferSize={cap.get(cv2.CAP_PROP_BUFFERSIZE)} FPS={cap.get(cv2.CAP_PROP_FPS)}")
        logger.info(f"DIAG: VideoCapture opened. Backend={cv2.CAP_FFMPEG} BufferSize={cap.get(cv2.CAP_PROP_BUFFERSIZE)} FPS={cap.get(cv2.CAP_PROP_FPS)}")
        logger.info(f"DIAG: VideoCapture opened. Backend={cv2.CAP_FFMPEG} BufferSize={cap.get(cv2.CAP_PROP_BUFFERSIZE)} FPS={cap.get(cv2.CAP_PROP_FPS)}")
        ...
        while session_manager.is_active(session_id):
            ...
            ret, frame = cap.read()
            _now = time.time()
            _gap = (_now - _last_frame_time) * 1000
            _last_frame_time = _now
            if frame_index % 100 == 0:
                _elapsed = _now - _diag_start
                _actual_fps = frame_index / _elapsed if _elapsed > 0 else 0
                logger.info(f"DIAG: frame={frame_index} ret={ret} gap={_gap:.1f}ms actual_fps={_actual_fps:.2f} freeze={freeze_count}")
            _now = time.time()
            _gap = (_now - _last_frame_time) * 1000
            _last_frame_time = _now
            if frame_index % 100 == 0:
