...
async def startup_event():
    ...
    try:
        ...
        logger.info(
            Messages.get(
                "API.STARTUP.004.INFO",
                deleted=result.get("deleted", 0),
                freed_mb=result.get("total_size_mb", 0),
            )
        )
        logger.debug("cleanup result details: %s", result)
    except Exception:
        logger.exception(Messages.get("API.STARTUP.005.ERROR"))
